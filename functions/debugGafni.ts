...
Deno.serve(async (req) => {
  ...
  if (mode === 'pskey') {
    ...
    // Common Sungrow inverter point IDs: AC power, DC power, daily yield, total yield, PV1-6 V/I, temp, status
    ...
    const [r1, r2] = await Promise.all([
      sgPost(base_url, '/openapi/getDeviceRealTimeData', conn.config, token, user_id, { ps_key_list: [PS_KEY], device_type: 1, point_id_list: POINTS }),
      sgPost(base_url, '/openapi/getDeviceRealTimeData', conn.config, token, user_id, { sn_list: [SN], device_type: 1, point_id_list: POINTS }),
    ]);
    return Response.json({ psKey_result: r1, sn_result: r2 });
  }
