...
Deno.serve(async (req) => {
  ...
  if (mode === 'pskey') {
    // Try getDeviceRealTimeData with ps_key_list
    const [r1, r2, r3, r4] = await Promise.all([
      sgPost(base_url, '/openapi/getDeviceRealTimeData', conn.config, token, user_id, { ps_key_list: [PS_KEY] }),
      sgPost(base_url, '/openapi/getDeviceRealTimeData', conn.config, token, user_id, { ps_id: PS_ID, ps_key_list: [PS_KEY] }),
      sgPost(base_url, '/openapi/getDeviceRealTimeData', conn.config, token, user_id, { sn_list: ['A2240135735'] }),
      sgPost(base_url, '/openapi/queryDeviceRealTimeData', conn.config, token, user_id, { ps_id: PS_ID, ps_key: PS_KEY, point_id_list: [13003, 13119, 13009, 13010, 13011] }),
    ]);
    return Response.json({ ps_key_list_only: r1, ps_id_and_key: r2, sn_list_guess: r3, queryDeviceRT_with_psKey: r4 });
  }
